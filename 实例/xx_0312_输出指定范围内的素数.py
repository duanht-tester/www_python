...
# 作者算法中未使用到"开根号法"来节约时间复杂度，同时为加入个数等说明，因此我对其改进如下：
# import math
#
# # 输出指定范围内的素数
# # 用户输入范围
...
# prime_number = [x for x in range(int(input('区间最小值：')), int(input('区间最大值：')))
#                 if [] ==[y for y in range(2, int(x**0.5) + 1) if x % y == 0]]
# print(prime_number)
...
# 加上注释如下，并对负数进行改进，如果需要，还可以自行添加对其他字符串进行限定输入。
    ...
    if num > 1:  # 素数大于 1
        ...
        for i in range(2, (square_num + 1)):   # （2,2）属于空集，不会出错，但也不会执行
            if num % i == 0:  # 可以被整除说明是合数
                ...
                sumhs += num
                print(num, "是合数")
                break  #执行到这里说明是合数，跳出里层for和所
                # 有else语句，执行完其他语句（比如下面注释掉的）后继续外一层for的下一次循环
            else:  # 不能被依次整除，说明循环完了还是质数，用pass表示占位
                pass  # 继续执行下一句else语句
#这里的else和上面的for属于一个级的，能执行下面的语句说明上面的质数筛选
# 已经过关了，没有执行break。
